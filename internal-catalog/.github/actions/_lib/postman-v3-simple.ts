...
export interface PostmanCollectionV2 {
    info?: {
        name?: string | null;
        description?: PostmanDescription;
...
function buildUniqueRef(baseName: string, kind: 'folder' | 'request', usedRefs: Set<string>): { ref: string; name: string } {
...
function transformKeyValue(entry: PostmanKeyValue, options?: { allowNullValue?: boolean }): KeyValueDescriptor | QueryParamDescriptor {
...
function transformScripts(events: PostmanEvent[] | null | undefined, scope: 'collection' | 'request'): ScriptDescriptor[] | undefined {
    const listenMap: Record<string, string> = scope === 'collection'
        ? {
    ...
    const scripts = asArray(events)
        .map((event) => {
            ...
            const execLines = Array.isArray(script.exec)
                ? script.exec.map((line) => String(line)).filter((line) => line.trim().length > 0)
...
function transformResponses(responses: PostmanResponse[] | null | undefined): ResponseExampleDescriptor[] | undefined {
    const examples = asArray(responses).map((response) => {
        ...
        const headers = asArray(response.header).map((entry) => transformKeyValue(entry) as KeyValueDescriptor);
        if (headers.length > 0) {
...
function createBaseNode(item: {
    name?: string | null;
    description?: PostmanDescription;
    id?: string | null;
    auth?: PostmanAuth | null;
    event?: PostmanEvent[] | null;
}, scope: 'collection' | 'request', fallbackName: string): BaseNode {
    const node: BaseNode = {
...
function transformRequestNode(item: PostmanItem): RequestNode | null {
    ...
    const headers = asArray(request.header).map((entry) => transformKeyValue(entry) as KeyValueDescriptor);
    if (headers.length > 0) {
        node.headers = headers;
    }

    if (request.url && typeof request.url === 'object') {
        const queryParams = asArray(request.url.query).map((entry) => transformKeyValue(entry, { allowNullValue: true }) as QueryParamDescriptor);
        if (queryParams.length > 0) {
            node.queryParams = queryParams;
        }
        const pathVariables = asArray(request.url.variable).map((entry) => transformKeyValue(entry) as KeyValueDescriptor);
    ...
    const examples = transformResponses(asArray(request.response).length > 0 ? request.response : item.response);
    if (examples) {
...
function transformCollection(collection: PostmanCollectionV2): CollectionNode {
    ...
    const variables = asArray(collection.variable).map((entry) => transformKeyValue(entry) as KeyValueDescriptor);
...
function buildBaseDescriptor(node: BaseNode, type: 'collection' | 'folder'): Record<string, unknown> {
...
async function writeRequestNode(node: RequestNode, parentDir: string, usedRefs: Set<string>): Promise<{ ref: string }> {
    const { ref } = buildUniqueRef(node.name, 'request', usedRefs);
    await fs.writeFile(
        path.join(parentDir, ref.replace('./', '')),
        stringify(buildRequestDescriptor(node)),
        'utf8'
    );
    return { ref };
}

async function writeFolderNode(node: FolderNode, parentDir: string, usedRefs: Set<string>): Promise<{ ref: string }> {
    ...
    await fs.writeFile(path.join(folderDir, 'folder.yaml'), stringify(descriptor), 'utf8');
    return { ref };
}
...
export async function convertAndSplitCollection(v2Collection: PostmanCollectionV2, outputDir: string): Promise<void> {
    ...
    await fs.writeFile(path.join(outputDir, 'collection.yaml'), stringify(descriptor), 'utf8');
}
